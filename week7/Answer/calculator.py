...
class Calculator(QWidget):
    def __init__(self):
        ...
        # 내부 표현식 저장
        self._expr = ""
        self._isResulted = False # 현재 화면에 표기된 값이 연산결과값인지 확인 목적
        self._create_ui()
    ...
    def divide(self, a, b):
        """b가 0이면 ZeroDivisionError를 발생시키고, 아니면 나눗셈 결과를 반환합니다."""
        if b == 0:
    ...
    def percent(self):
        """현재 숫자를 100으로 나눠 퍼센트 값을 표시."""
        if not self._expr:
            return
        try:
            value = float(self._expr) / 100
            self._expr = str(value)
            self.display.setText(self._format_result(value))
        except:
            pass
    ...
    # ==== 결과 출력 메소드 - 보너스 문제 참조 ====
    def equal(self):
        """내부 표현식을 계산하고 결과를 표시합니다. 예외 발생 시 'Error' 출력."""
        try:
            expr = (self._expr
                    .replace('×','*')
                    .replace('÷','/')
            ...
            # 1. error 문자열이 반환되는 경우 캐치
            # 2. 계산 도중 result의 값이 유실되는 경우 캐치(None)
            # 3. float 형 result에서 범위를 벗어난 양/음 무한대가 나타나는 경우 캐치
            if result == 'error' or result is None or result in (float('inf'), float('-inf')):
                raise Exception()
            # 소수점 자리 및 폰트 크기 조정 포함 포맷팅
    ...
    def _format_result(self, value: float):
        """
        결과값을 최대 소수점 6자리로 반올림하고,
        출력 길이에 따라 폰트 크기를 동적으로 조정합니다.
        """
        # 소수일 경우 반올림
